    ...
    const renderUploadStep = () => (
        <div className="border-2 border-dashed border-muted-foreground/50 rounded-lg p-12 text-center">
            ...
            <Button asChild className="mb-2">
                <Label htmlFor="student-upload-file">
                    {isParsing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <UploadCloud className="mr-2 h-4 w-4"/>}
                    {isParsing ? '解析中...' : '選擇檔案'}
                    <Input id="student-upload-file" type="file" className="sr-only" onChange={handleFileChange} accept=".xls,.xlsx" disabled={isParsing} />
                </Label>
            </Button>
