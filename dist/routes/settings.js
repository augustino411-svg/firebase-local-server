...
function calculateSchoolDays(start, end, holidays, extraHolidays) {
...
router.post('/', async (req, res) => {
    const { academicYear, semester, label, startDate, endDate, holidays, holidayDates, permissions, } = req.body;
    if (!academicYear || !semester || !startDate || !endDate) {
        return res.status(400).json({ message: '缺少必要欄位' });
    }
    try {
        const totalSchoolDays = calculateSchoolDays(new Date(startDate), new Date(endDate), holidays || [], holidayDates || []);
